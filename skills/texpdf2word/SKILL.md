...
5b. Add cross-reference navigation (recommended before finalizing):
   - Run `scripts/add_crossref_hyperlinks.py final.docx` so that in-text 图/表/式/代码清单 references and `[N]` citations jump to their caption, equation, listing, or bibliography targets. Every jump target must exist; the script reports tokens without targets instead of guessing.
   - After finalization, verify in COM that every internal hyperlink reports `Address == ''`; if any show `Address == 'h'` (the `w:history` pitfall), run `scripts/add_crossref_hyperlinks.py final.docx --fix-citation-fields` and re-export the PDF.
5c. Verify table grid structure (before and after finalizing):
   - Run `python scripts/fix_table_structure.py final.docx --audit-only` — every table row must cover exactly its `tblGrid` columns (missing `gridSpan` on multicolumn headers, extra multirow filler cells, and phantom grid columns are the recurring LaTeX→Word failures). Repair with the same script without `--audit-only`; borders and vertical merges are never modified.
6. Automated and visual verification:
   - Use `scripts/validate_docx_conversion.py final.docx --template template.docx --protected-until "中 文 摘 要" --pdf final.pdf --out validation.json` for placeholder/order/header/image/table checks plus protected-front-matter format checks. Choose the real first generated marker of the target template.
...
- **Citations rendered as `(Author Year)` instead of GB/T-7714 `[N]` superscript.** Pandoc with default `--citeproc` (or no citeproc at all when the source uses `\cite`) emits `(Zhang et al. 2025)`-style author-year tokens — visible but **not** a Word hyperlink and **not** the numeric format Chinese thesis templates require. The structural validator counts paragraphs and reference bookmarks; it does not see that the in-text mark format is wrong. → After pandoc, parse `main.bbl` for `\bibitem{key}` order to build a `cite_key → number` map and a per-key `(first_surname, year)` token; walk every paragraph in `document.xml` looking for `(Author Year[; Author Year]*)` patterns; replace each with `[N1, N2]` wrapped in `<w:hyperlink w:anchor="ref_N">` runs that carry `<w:vertAlign w:val="superscript"/>`. **Disambiguate same-(surname,year) collisions** (e.g. two `Wang 2024` or two `Li 2025` entries) by combining the bbl opt's secondary author list **and** the first author's initial from the bbl author line — pandoc emits `(W. Li et al. 2025)` vs `(J. Li et al. 2025)` exactly to disambiguate. Use word-boundary `\b<initial>\b` matching so single letters do not accidentally match inside `et al.`. Detected by the `citation-coverage` check.
...
2.  **Disambiguation rules need a unit test before commit.** When a citation
    ...
    `assert disambig("(Wang et al. 2024)") == "wang2024"`. The
...
- `scripts/add_crossref_hyperlinks.py`: add clickable cross-reference navigation to a converted DOCX. Pass 1 bookmarks every target paragraph — figure captions (`图N-M …`), table captions (`表N-M …`), equation-number paragraphs whose visible text is `(N-M)`, listing titles (`代码清单N-M …`), and reference entries (`[N] …`) — using Word-safe bookmark names (`figref_N_M`, `tabref_N_M`, `eqref_N_M`, `lstref_N_M`, `ref_N`; underscores because Word bookmark names cannot contain `-`). Pass 2 merges adjacent plain runs (tokens like 图+2-2 usually span two runs), then splits runs and wraps every in-text token — `图N-M`, `图N-M(a)`, `表N-M`, `式N-M`, `式(N-M)`, `(N-M)`, `代码清单N-M`, and with `--link-citations` plain `[N]` citations — in `<w:hyperlink w:anchor="…">`, preserving run formatting. Tokens already inside a hyperlink (e.g. citations produced by the project pipeline) and tokens inside OMML math are untouched, and a caption paragraph's own leading number is never linked to itself. `--fix-citation-fields` repairs fields that Word serialized as `HYPERLINK \l "x" \h` (see the `w:history` pitfall below). Run BEFORE `finalize_word_docx.py` (Word converts the inserted `w:hyperlink` elements into HYPERLINK fields on save, which also survive PDF export as internal links).
- **`w:history` pitfall (breaks citation jumps silently).** Every internal `<w:hyperlink w:anchor="…">` element MUST carry `w:history="1"`. When Word saves, elements WITHOUT the attribute are serialized as `HYPERLINK \l "x" \h` fields; Word parses the stray `\h` as an external target, the link reports `Address='h'`, and Ctrl+Click tries to open a file named `h` instead of jumping — while sibling links that DO carry `w:history="1"` serialize as clean `HYPERLINK \l "x"` and work. Symptom: only some internal links (often exactly the citations) refuse to jump. Verify in COM: a healthy internal link has `Hyperlink.Address == ''` and a non-empty `SubAddress`; run `add_crossref_hyperlinks.py --fix-citation-fields` after finalization to strip the stray `\h` from already-saved documents (PAGEREF `\h` switches are legitimate and left untouched).
- `scripts/render_pdf_preview.py`: renders selected PDF pages into contact sheets for visual QA.
- `presets/chinese_thesis.json`: generic style/table/hyperlink config sample for a typical Chinese thesis template. It does not delete sample pages, fill cover fields, replace abstracts, rebuild TOC, or repair section headers by itself.

## References

- Read `references/pandoc-limitations.md` when explaining why this workflow differs from pandoc defaults.
- Read `references/thesis-case-study.md` when building or adapting a university-thesis pipeline.
