# Thesis Template Case Study

This reference summarizes the proven pattern from a real Chinese-university
LaTeX-thesis-to-Word conversion. Specific names are anonymized.

## Input

- Source: a multi-file LaTeX thesis project with `main.tex`, `data/*.tex`,
  `figures/*`, and BibTeX references.
- Template: the university's official undergraduate thesis `.docx` template.
...
- **3 figures missing.** Three chapters' `\includegraphics{thesis_structure}`,
  `\includegraphics{method_framework}`, and `\includegraphics{simulation_flow}`
  all reference the basename
  ...
  *Mitigations:* run `validate_docx_render.py --source-latex-dir
  <latex-project>/data` to assert source `\includegraphics` count ≤ rendered
...
- **Citations rendered as `(Author Year)`, not GB/T-7714 `[N]` superscript.**
  Pandoc's default cite-rendering produces tokens like `(Zhang et al. 2025)` /
  `(Chen et al. 2024; Zhang et al. 2025)`. They are **visible**, so a paragraph
  ...
  collisions like two `wang 2024` entries by matching the bbl opt's
  secondary author list **plus the first author's initial from the bbl
  author line** — pandoc emits `(W. Li et al. 2025)` vs `(J. Li et al. 2025)`
  when two cite_keys share the same surname-year, and `(Wang and
  Chen 2024)` vs `(Wang et al. 2024)` is distinguishable by
  whether the secondary surname `Chen` actually appears in the docx token.
