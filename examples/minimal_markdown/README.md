...
└── expected/
    └── README.md              ← 说明 final.pdf / 预览拼图如何生成与对比
```
...
**真实学位论文模板**上的输出，传一个本地的 `.docx` 模板路径即可：
...
生成出来的正文页（真实插图、图题、正文段落、显示公式和三线表）。

模板本身不会被 commit（已在 `.gitignore` 里），运行产物也只会写到
`examples/minimal_markdown/`。出于隐私考虑，仓库不预置任何基于真实
学校模板生成的截图；如何自己生成并对比，详见
...
仓库不预置任何成品截图（避免携带真实学校模板的内容）。想直接看产出
长什么样，在 Windows + Word + pywin32 的环境里跑一遍，即可得到
